...
description: "A deep dive into OpenClaw's three-tier memory system: hybrid search with nomic-embed-text embeddings, temporal decay, sqlite-vec vector storage, and the full configuration that makes agents remember across sessions."
tags: ["Claude Code", "OpenClaw", "Memory", "Ollama", "Multi-Agent", "sqlite-vec"]
...
*Written by Chris Johnson. The configuration shown is the actual memory architecture running on the JClaw multi-agent system on an M4 Mac Mini. Technical details sourced from the [memory-architecture.md](https://github.com/chris2ao/JClaw_Config) documentation in the JClaw_Config repository. Part 1 is [here](/blog/openclaw-deployment-part-1-the-fortress-that-locked-itself) and Part 2 is [here](/blog/openclaw-deployment-part-2-the-comeback).*
