...
Prompt Injection and Jailbreaking was my weakest section, and the section average turned out to be the least useful number on the page. Underneath it sat four separate sub-scores, one for each variant the section tests: direct injection, indirect injection, and each of those again with guardrails in the loop. The spread between them was enormous.

That spread, not the average, is what told me where my skill stopped and my instinct started. I'm keeping the numbers themselves to myself, since they describe my transcript rather than the exam, but the shape of them is worth writing down.

On direct injection I was good, and I mean that without hedging. I built a coherent multi-turn narrative and sustained it. Every small concession the model granted became leverage for the next request. That reads as strategy when it works, not guesswork, and it was comfortably my strongest sub-score of the four.

It's the version of me I'd want writing the authorization code I described a section ago: deliberate, watching what the previous move bought, building on it.

Indirect injection is where I fell apart, and the reason stings a little, because it's a debugging failure rather than a technique failure.

Against consistent refusals, I cycled through techniques instead of reading the failure signal and diagnosing which layer was actually blocking me. I'd hit a wall, try a different angle, hit the same wall from a slightly different position. I kept rotating instead of stopping to ask which specific control was firing.
