...
coverImageAlt: >-
  A dark teal cover graphic titled Ledgerly, Part 2, A Chat That Can Query
  Its Own Database, with the kicker ten read-only tools, twelve turns, the
  same SQLite file. Four panels describe the build: MCP Server, roughly 250
  lines of hand-rolled JSON-RPC over stdio with no SDK, newline-delimited
  JSON-RPC 2.0 across 4 files; 10 Tools, read-only wrappers for summaries,
  trends, budgets, and merchants, each one wrapping a query function that
  already existed; Zero Shell, confined with --strict-mcp-config and empty
  --tools and --setting-sources flags, no shell, no built-in tools, one
  allowed tool; and 1,054 Rows, wrongly excluded transactions restored
  after the chat noticed them while answering an unrelated question. Below
  the panels, a tagline reads "Ten tools. Twelve turns. A database it can
  query. It found 1,054 real transactions nobody asked about."
---
