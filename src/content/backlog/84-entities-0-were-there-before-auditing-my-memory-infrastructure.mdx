---
title: "84 Entities, 0 Were There Before: Auditing My Memory Infrastructure"
date: "2026-03-29T10:00:00"
description: >-
  I ran a routine audit on my Gmail agent and discovered the knowledge graph
  had been completely empty for months, the memory nudge hook had five blind
  spots, and nothing was catching either problem. Here's what I built to fix it.
tags:
  - Claude Code
  - Memory
  - Workflow
...
This post is about the weekend I spent finding those gaps and building the feedback loops to make sure they don't happen again. No new features shipped. No users noticed a difference. But the infrastructure that supports my daily development is measurably more reliable now, and the things that were silently failing aren't silent anymore.
...
```mermaid
flowchart TD
    subgraph L1["Layer 1: Auto Memory"]
        AM["MEMORY.md files"]
    end
    subgraph L2["Layer 2: Vector Memory"]
        VM["MCP Server (semantic search)"]
    end
    subgraph L3["Layer 3: Knowledge Graph"]
        KG["MCP Server (entity relations)"]
    end
    subgraph L4["Layer 4: Homunculus"]
        HM["Hooks + Instincts"]
    end
    subgraph L5["Layer 5: Session Archive"]
        SA[".claude/session_archive/"]
    end

    L1 -.- |"Per-project stable facts"| L2
    L2 -.- |"Global narrative context"| L3
    L3 -.- |"Structural relationships"| L4
    L4 -.- |"Behavioral patterns"| L5
    ...
    style L5 fill:#c3fae8,stroke:#06b6d4
```
...
```mermaid
pie title Knowledge Graph: 84 Entities by Category
    "Agents (20)" : 20
    "Scripts (12)" : 12
    "MCP Servers (11)" : 11
    "Hooks (9)" : 9
...
```mermaid
flowchart LR
    subgraph before["Before: 5 Blind Spots"]
        direction TB
        B1["Extensions: .ts .js .py .json .sh .yaml"]
        B2["Tools: Edit | Write | Bash | Agent"]
        B3["Work Type: Writes only"]
        B4["Timing: Threshold only"]
        B5["Language: 'consider saving...'"]
        style B1 fill:#ffc9c9,stroke:#ef4444
        style B2 fill:#ffc9c9,stroke:#ef4444
        style B3 fill:#ffc9c9,stroke:#ef4444
        style B4 fill:#ffc9c9,stroke:#ef4444
        style B5 fill:#ffc9c9,stroke:#ef4444
    end

    subgraph after["After: Patched"]
        direction TB
        A1["Extensions: + .md .mdx"]
        A2["Tools: + mcp__ prefix"]
        A3["Work Type: + Read/Grep/Glob at 0.5x"]
        A4["Timing: + Stop hook checkpoint"]
        A5["Language: 'MEMORY SAVE REQUIRED'"]
        style A1 fill:#b2f2bb,stroke:#22c55e
        style A2 fill:#b2f2bb,stroke:#22c55e
        style A3 fill:#b2f2bb,stroke:#22c55e
        style A4 fill:#b2f2bb,stroke:#22c55e
        style A5 fill:#b2f2bb,stroke:#22c55e
    end
...
```mermaid
flowchart TD
    SESSION["Claude Code Session"] --> L1["Layer 1: memory-nudge.sh\n(PostToolUse hook)"]
    SESSION --> L2["Layer 2: memory-checkpoint.sh\n(Stop hook)"]

    L1 --> |"Fires at threshold\n(3+ work units)"| NUDGE["MEMORY SAVE REQUIRED\n(mid-session reminder)"]
    L2 --> |"Fires once at\nsession end"| CHECK["5-Category Checklist\n Tasks / Decisions / Bugs\nGotchas / Errors"]

    NUDGE --> VM["Vector Memory\n(mcp__vector-memory)"]
    CHECK --> VM

    L1 -.- |"Catches"| F1["Long sessions that\nforget to save"]
    L2 -.- |"Catches"| F2["Short sessions that\nend before threshold"]
    L2 -.- |"Catches"| F3["Sessions where nudge\nwas deprioritized"]

    style L1 fill:#a5d8ff,stroke:#4a9eed
    style L2 fill:#d0bfff,stroke:#8b5cf6
    style NUDGE fill:#fff3bf,stroke:#f59e0b
    style CHECK fill:#fff3bf,stroke:#f59e0b
    style VM fill:#b2f2bb,stroke:#22c55e
    style F1 fill:#ffc9c9,stroke:#ef4444
    style F2 fill:#ffc9c9,stroke:#ef4444
    style F3 fill:#ffc9c9,stroke:#ef4444
```
...
```mermaid
flowchart LR
    CHANGE["Component Changed\n(agent, hook, skill...)"] --> DETECT["kg-update-detect.sh\n(PostToolUse hook)"]
    DETECT --> |"Reminder:\nKG may be out of sync"| DECIDE{Small or\nlarge change?}
    DECIDE --> |"Small"| MANUAL["Direct API call\ncreate_entities\ncreate_relations"]
    DECIDE --> |"Large / Uncertain"| SYNC["/Knowledge-Graph-Sync\n(5-phase reconciliation)"]

    SYNC --> S1["1. Disk inventory"]
    SYNC --> S2["2. KG inventory"]
    S1 --> S3["3. Reconcile entities"]
    S2 --> S3
    S3 --> S4["4. Validate relations"]
    S4 --> S5["5. Report + confirm"]

    MANUAL --> KG["Knowledge Graph\n(up to date)"]
    S5 --> KG

    style CHANGE fill:#ffd8a8,stroke:#f59e0b
    style DETECT fill:#a5d8ff,stroke:#4a9eed
    style SYNC fill:#d0bfff,stroke:#8b5cf6
    style KG fill:#b2f2bb,stroke:#22c55e
    style MANUAL fill:#b2f2bb,stroke:#22c55e
```
...
**Maintenance sessions produce value that's hard to see.** No new features shipped this session. No user-visible changes happened. But the systems that support ongoing development are more reliable, the memory infrastructure is now actually functioning at all layers, and future sessions will accumulate context more consistently. The value is real; it's just diffuse and delayed.
...
The infrastructure that supports my work is now actually doing what it was designed to do. That's a good day's work, even if nothing visible shipped.
