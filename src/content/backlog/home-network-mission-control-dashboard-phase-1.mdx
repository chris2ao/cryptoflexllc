...
description: >-
  12 parallel workstreams, then four enrichment waves on top. 497 backend tests
  at 82.9 percent coverage. 132 frontend tests. One CRITICAL plus three HIGH
  security findings caught and fixed in the first review; one HIGH and three
  MEDIUM caught by an architect-agent review in the second. A read-only dashboard
  for my UDM Pro, Pi-hole, and UniFi Protect home lab, built in one long session
  against a design spec written beforehand, then dog-fooded and refined into
  what Phase 1 Mode A actually looks like in production.
tags:
  - Home Lab
  - FastAPI
  - React
...
The initial 12-workstream build used a wave-based pattern with up to four parallel Task agents per wave. The enrichment waves kept the wave pattern but added explicit personas: a Captain who owned sysadmin + graphic-designer scope (DB migration, LaunchAgent plists, Tailwind tokens, Tooltip primitive, chart-type selections), a Security Engineer who owned signal schemas + remediation copy + security gates, a Network Engineer who owned poll jobs + per-client enrichment + the DNS tab, and a Researcher who owned pattern validation (Grafana, Pi-hole v6 admin, Radix) + the chart primitives.
...
Phase 1.1 is a Wi-Fi + RF tab. Signal strength heatmap by AP, channel usage, client distribution per band. Benefits from the UniFi session-auth follow-up because traffic-flow data per client feeds the RF picture, so the session-auth plan should land first.

Phase 2 is the mutation unlock (A to B): preview-then-confirm on every write endpoint (same pattern as `chris2ao-unifi-mcp`'s Tier 2/3 tools), stricter rate limiting on mutations, an `audit_log` SQLite table capturing operator plus preview diff plus outcome, confirm-modal UI, and a security review on the new surface. The CSRF-token plumbing that the session-auth follow-up adds is a pre-req here.
