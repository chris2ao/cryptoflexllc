...
description: "A fourth-party supply-chain breach put nine of my production credentials in an attacker's hands. Twenty minutes after reading the disclosure I was rotating keys, and sixty minutes later I had a full audit and a hardened account. Here is how Claude Code turned a day of incident response into an hour, and why the chain that got here should change how you pick vendors."
...
I host cryptoflexllc.com and a game dev project on Vercel. On April 19, 2026, I read [Juliet.sh's writeup](https://juliet.sh/blog/vercel-april-2026-incident-what-customers-should-do) of the Vercel breach over morning coffee. Twenty minutes later I was logged into my Vercel dashboard looking at nine environment variables with a "Need To Rotate" badge on each of them. Sixty minutes after that, they were all rotated, the platform was hardened, and I had forensic proof nothing had been used against me.

This is the story of how Claude Code made that possible, why the breach chain that got here should worry anyone running production on a third-party platform, and why a SOC 2 badge is a claim, not a proof.
...
Nine credentials. One hour. Zero unauthorized access detected.
...
You are first party. Your direct vendors (the ones you pay) are second party. Their vendors (the ones you never signed a contract with) are third party. And their vendors are fourth party. Each layer is one more organization that can be compromised without you even knowing they are in the chain.
</Info>

The attacker who got to my Vercel env vars never touched my laptop. They were five organizations removed from me. Here is the chain as best as it has been reconstructed from public reporting:
...
3. From that foothold, an attacker moved to a Vercel employee's Workspace. The bridge was consumer OAuth: the Vercel employee had individually signed up for Context.ai's consumer product using their enterprise Google account and granted "Allow All" permissions. That token was replayed.
4. With a valid Vercel-employee session, the attacker enumerated non-sensitive environment variable values across a subset of customer projects between March 2026 and April 19, 2026.
5. I was one of the customers in that subset.

<SupplyChainAttackDiagram />

That is what supply chain exposure looks like in 2026. The attacker never touched my Mac. They were five degrees of separation away, and still my production keys were readable.

![Slide: Five Degrees of Separation. A four-stage chain showing Context.ai Employee Personal Device (Lumma Stealer infection via disguised Roblox script, $250/mo commodity malware) to Context.ai Google Workspace (browser session cookie stolen) to Vercel Employee (consumer OAuth replayed with Allow All permissions via Context.ai) to Vercel Customer Environment Variables (attacker enumerates unprotected keys via internal tooling). Status note: the attacker never touched the local environment. Production keys were readable entirely through fourth-party supply chain vulnerabilities.](/blog/vercel-breach-incident-response/slides/slide-02.png)

<Warning title="The Sensitive flag was the only thing that would have saved me">
Vercel offers a "Sensitive" flag on every environment variable. When it is on, the value is encrypted at rest and not enumerable through the internal tooling the attacker got to. When it is off, the value is stored in a form that internal tooling can read. None of my nine compromised variables had the Sensitive flag set. The flag is architecturally enforced, not policy-enforced. It was the only thing standing between "breach" and "not breach" for me, and I did not know that until the disclosure came out.
</Warning>

## The Screenshot I Did Not Want To See

Here is what greeted me when I opened my Vercel Environment Variables page after reading the Juliet.sh writeup.
...
Nine of my thirteen environment variables, each flagged by Vercel as "Need To Rotate". The other four were not flagged because they are public identifiers (team ID, GA ID, project ID) or an email address. Vercel's own UI was telling me exactly what to do. The question was how fast I could do it, and how confident I could be that nothing had already been used.
...
curl -sS -H "Authorization: Bearer $NEW_VERCEL_API_TOKEN" \
  "https://api.vercel.com/v3/events?teamId=$TEAM_ID&limit=12&since=1738368000000"
...
![Infographic: The 60-Minute Remediation, AI-Assisted Recovery from the Vercel Breach. Four panels. Anatomy of a 4th-Party Breach shows the five-stage chain from Lumma Stealer infection to Vercel customer environment variable exposure. AI vs Manual Remediation claims an 800 percent increase in recovery speed (one hour with Claude Code versus six to eight hours manual). Rotation Ladder and Forensic Proof shows the priority order VERCEL_API_TOKEN then Database URLs then HMAC Secrets, with programmatic verification against 100 deployments and 12 team events yielding zero unauthorized access events. Hardening and Lessons Learned lists the Sensitive flag enablement, 2FA mandate, Deployment Protection at Standard, and the 7-day post-rotation monitoring window.](/blog/vercel-breach-incident-response/infographic.png)
...
This ordering fell out of a two-minute conversation with Claude Code: "What order should I rotate these in, and why?" The answer was correct, and the explanation was good enough that I could sanity-check it and push back on one item I disagreed with. (I bumped `DATABASE_URL` to position 2 instead of 3 because Neon downtime during a password reset is the most disruptive thing in the chain for real users, so I wanted to get it done early rather than leave it for the back half of the window.)
...
Six of the rotations were UI-gated. Open the issuer's dashboard, click buttons, copy a new value. But three of them were pure signing keys generated by me, for me, used by my own code:
...
Claude Code ran that one-liner, printed the three new values, and then piped them into `vercel env add ... --sensitive`. Three fully scriptable rotations in under thirty seconds, each with the Sensitive flag turned on this time.
...
Every state change was verified programmatically. After enabling 2FA, Claude Code hit the Vercel API to confirm my account reported MFA enabled. After rotating `DATABASE_URL`, it ran `psql "<old-connection-string>"` and confirmed it now fails with an authentication error. Trust, but verify. Better: do not trust the UI, verify the API.
...
If you host anything on Vercel, read this section and act on it. I have tried to make it scannable enough to execute on a phone.
...
2. Flag every future env var as **Sensitive**. The flag is architecturally enforced (encrypted at rest, not enumerable through the compromised channel).
...
5. Delete all Deployment Protection bypass tokens you are not actively using.
6. If you use Google Workspace, audit every third-party OAuth grant at [admin.google.com](https://admin.google.com) (Security to API controls to Manage Third-Party App Access). Revoke anything you do not recognize.
7. Switch your Workspace third-party app policy from default-allow to "only trusted apps" (explicit allowlist mode). This is a one-time setting with large payoff.
8. Monitor upstream services for 7 days post-rotation. If anyone attempts to auth with your revoked credentials, the stolen value was real. Services to watch for my stack: Neon, Anthropic, Resend, GitHub, Gmail, Vercel.
</Security>
...
- **1** old Vercel API token revoked (the one that could have been used against me).
- **6** Google Workspace third-party OAuth apps revoked in the hardening pass.
- **0** unauthorized access events found in 100 deployments or 12 events.
...
- **~6 to 8** hours, optimistically, if I had done the same work without Claude Code.
...
## Why This Is What "AI For Defenders" Actually Looks Like
