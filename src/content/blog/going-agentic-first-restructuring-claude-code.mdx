...
One of my biggest concerns: Claude accidentally editing `.env` files, private keys, or certificate files. I built a PreToolUse hook that intercepts every Edit and Write operation and blocks it if the target is a sensitive file.
...
The hook intercepts the JSON payload that Claude Code sends on stdin (containing the tool name and parameters), extracts the target file path, and checks it against blocklists. The `catch` block exits with 0 (allow) so that hook errors never accidentally block legitimate operations.
...
Throughout the session, `~/.claude.json` kept throwing "file modified since read" errors when I tried to edit it. The cause: Claude Code itself modifies this file (updating MCP state, session metadata), creating a race condition between my edits and its internal writes. The solution: use `claude mcp add/remove` commands instead of manual JSON editing, and let Claude Code manage its own config file.
