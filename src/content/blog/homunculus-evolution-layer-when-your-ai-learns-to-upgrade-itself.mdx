...
```mermaid
flowchart TD
    A["PostToolUse Hooks"] -->|"Every tool call"| B["observations.jsonl\n(13,292 entries, 2.7 MB)"]
    B -->|"Observer agent"| C["Instincts Directory\n(50 markdown files)"]
    C -->|"/evolve command"| D["Evolve Synthesizer Agent"]
    D -->|"Semantic clustering"| E["Evolved Candidates\n(agents, skills, commands)"]
    E -->|"User review"| F{"Accept or\nDecline?"}
    F -->|"Accept"| G["evolved/ directory"]
    G -->|"promote-evolved.sh"| H["Active directories\n(~/.claude/skills/, agents/, commands/)"]
    F -->|"Decline"| I["Discarded"]
    ...
    subgraph "Layer 2: Instincts"
        C
    end

    subgraph "Layer 3: Evolution (NEW)"
        D
        E
        F
        G
...
```mermaid
flowchart LR
    A["50 Instincts"] --> B["Evolve Synthesizer"]
    B --> C{"Semantic\nClustering"}
    C --> D["13 Clusters"]
    C --> E["13 Unclustered\n(too narrow/low confidence)"]
    D --> F["9 Skills"]
    D --> G["4 Commands"]
    F --> H{"User\nReview"}
    G --> H
    H -->|"7 Accepted"| I["5 Skills Promoted"]
    H -->|"6 Declined"| J["Removed"]
    H -->|"2 Commands"| K["Declined and Removed"]
