...
description: >-
  What happens when you ship 90% context reduction in the morning and build a
  full backlog staging system by afternoon? A look at a typical day building
  with Claude Code.
date: '2026-02-21'
tags:
  - Claude Code
...
3. `src/app/backlog/_components/post-action-bar.tsx` - Publish/delete state machine (170 lines)
   - Publish button → POST to `/api/backlog/publish`
   - Delete button → DELETE to `/api/backlog/[slug]`
...
- From `/analytics`: "View backlog drafts →"
- From `/backlog`: "← Back to Analytics"
...
5. **Iterate on related features** (comments moderation → deep linking → backlog staging)
