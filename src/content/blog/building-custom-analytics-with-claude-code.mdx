...
After deploying, enable it in your Vercel project dashboard under **Analytics** > **Web Analytics**. The toggle and the component are both required — one without the other won't work.
...
Here's the catch: **ad blockers kill both of these.** uBlock Origin, Brave shields, and most privacy extensions block Vercel's analytics scripts (`/_vercel/insights/script.js` and `/_vercel/speed-insights/script.js`) the same way they block Google Analytics. In my testing, I saw `ERR_BLOCKED_BY_CLIENT` in the DevTools Network tab — the scripts never load.
...
**Why ad blockers don't catch this:** This is a same-origin POST to your own API route. It doesn't load an external script, it doesn't hit a third-party domain, and it doesn't match the filter lists that ad blockers use. The request looks identical to any other API call your application makes.
...
The `${ip}` in the template literal is NOT string interpolation — it's a parameter placeholder that gets sent separately from the SQL string, preventing SQL injection.

## The Dashboard: Visualizations

The original dashboard was all HTML tables — functional but not easy to scan at a glance. I added interactive visualizations using [Recharts](https://recharts.org) (via shadcn/ui's chart component) and [Leaflet](https://leafletjs.com) (via react-leaflet) for a world map.
...
**Important SSR note:** Leaflet accesses the browser's `window` object, which doesn't exist during server-side rendering. You must wrap the map component with `next/dynamic` and `ssr: false`:
...
Rather than relying on users (including myself) to paste the right format, I added a sanitization step:
...
Now both formats work — raw URL or full CLI command.

### Problem 2: Environment Variables Need a Redeploy

This is a Vercel fundamental that caught me off guard: **changing an environment variable does NOT affect running deployments.** You must redeploy for the new value to take effect. The Vercel dashboard updates instantly, but your serverless functions still run with values baked in at build time.
...
- `ANALYTICS_SECRET` — a random string for dashboard access (`openssl rand -hex 32`)
...
This creates both the `page_views` table and the `ip_intel` cache table.
