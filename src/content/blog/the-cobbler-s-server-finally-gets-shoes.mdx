...
From their [official knowledge base](https://vercel.com/kb/guide/cloudflare-with-vercel):

> "Using Cloudflare's proxy mode in front of Vercel deployments is **not recommended** and may cause the following issues:
> - SSL certificate renewal conflicts
> - Degraded performance (double edge hop)
> - Broken bot protection
...
**The decision:** Vercel's native WAF. Zero infrastructure changes. Zero DNS migration. Zero risk of breaking SSL or bot protection. And it was already there, waiting to be configured.
...
1. **Host header injection protection.** If a request to `/api/*` contains the `x-forwarded-host` header, deny it. This header is used in cache poisoning and SSRF attacks. Legitimate requests from browsers don't send it. Attackers probing for vulnerabilities do.

2. **Dotfile access blocking.** Deny requests for `.env`, `.git`, `.svn`, `.htaccess`, `.htpasswd`, and `.DS_Store`. These files contain sensitive configuration data, version control history, and server directives. They should never be accessible over HTTP, but misconfigurations happen. This rule ensures they're blocked even if the server config is wrong.

3. **WordPress probe blocking.** Deny requests for `wp-admin`, `wp-login.php`, `wp-content`, `xmlrpc.php`, and `wp-includes`. My site doesn't use WordPress. These requests are 100% noise from automated scanners looking for vulnerable WordPress installs. Block them.

4. **Database admin panel blocking.** Deny requests for `phpmyadmin`, `pma`, `adminer`, and `mysqladmin`. Same logic as WordPress. My site doesn't have a database admin panel exposed to the web. Any request for these paths is a probe.
...
**The actual schema expects `"routes"`, not `"rules"`.** And inside each route object, the path goes in `"src"`, not `"route"`. The documentation example I'd found was either outdated or wrong. The official schema (referenced at the top of `vercel.json` via `"$schema": "https://openapi.vercel.sh/vercel.json"`) is the source of truth.
...
**Why this matters:** Schema validation errors can be cryptic when you're following documentation that's slightly off. Always check the schema directly if a config file has a `"$schema"` reference. That's your ground truth.
...
The recommended approach for new dashboard rules: start with **Log** action for 24-48 hours, monitor what gets matched, then upgrade to Deny or Challenge after confirming the rule doesn't block legitimate traffic.
...
This is exactly what you want from a WAF. Bots don't solve CAPTCHAs. Humans do. The WAF let me prove I was human, then stopped challenging me.
...
1. **Check what your existing infrastructure provides before adding a new service.** I almost migrated DNS to Cloudflare for a feature Vercel already had. Reading both vendors' docs saved me a migration and improved performance.

2. **Vendor recommendations matter.** When Vercel explicitly says "don't proxy through Cloudflare," that's not a suggestion. It's based on real observed issues: SSL conflicts, degraded performance, broken features. Trust the people who built the platform.

3. **Schema validation errors are your friend.** When a build fails with "should NOT have additional properties," that's the schema telling you the docs are wrong (or outdated). Check the `$schema` reference for ground truth.

4. **Test with the tools attackers use.** `curl` is how bots and scanners interact with your site. If your WAF doesn't block malicious curl requests, it won't block real attacks either. Test with realistic payloads.

5. **Persistent action is a feature, not a bug.** When the WAF started challenging my browser after I ran too many curl tests, that was exactly the behavior I wanted. Adaptive defenses that escalate when they see patterns are better than static rules.

6. **Free doesn't mean weak.** Vercel's free WAF is production-grade. DDoS mitigation, bot detection, custom rules, and attack challenge mode are all included. You don't need an enterprise contract to have real perimeter defense.
