...
seriesOrder: 3
coverImage: /blog/homenet-dashboard-phase-3/infographic.jpg
coverImageAlt: >-
...
![The plan doc as a self-contained engine: five sections (deferred queue, persona model, slice menu, phased designs, verification checklist) wired together so a fresh agent session can read state, fetch specs, load rules, follow the slice menu, and execute the verification checklist without a single clarifying prompt.](/blog/homenet-dashboard-phase-3/slides/slide-03.webp)
...
![The cold-handoff architecture: a fresh Claude session reads three inputs in parallel: the original plan doc (API and specs), the Codex wrap-up file (state and position), and the Codex diff (implementation). Instant context alignment, zero human prompting required.](/blog/homenet-dashboard-phase-3/slides/slide-07.webp)

![The two issues a fresh context window spotted instantly: ruff format CI red (a five-second mechanical fix that saves three hours of CI grief) and a PII consistency gap on the new domain endpoint (the real fix, where the new endpoint silently leaked unidentified clients' MAC addresses).](/blog/homenet-dashboard-phase-3/slides/slide-08.webp)
...
![Sweeping the deferred queue closes the loop: 20 starting items at the top of Phase 3, then Wave 1 backend hardening (3 items) and Wave 2 frontend polish (8 items) closed in the same session, leaving 9 items remaining (the explicit v2 / mode-B-gated work).](/blog/homenet-dashboard-phase-3/slides/slide-10.webp)
...
![Four immutable heuristics for AI engineering distilled from Phase 3: FORMAT_IS_PRE_CI, SIBLINGS_ARE_TRUTH, CLICK_THROUGHS_ARE_INTEGRATION, SWEEP_QUEUES_IN_SESSION.](/blog/homenet-dashboard-phase-3/slides/slide-13.webp)
