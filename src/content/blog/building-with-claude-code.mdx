...
Node.js is the JavaScript runtime that powers everything in modern web development. If you're coming from the Apache world, think of it as the engine that replaces your server-side scripting. It also ships with **npm** (Node Package Manager), which is how you install every dependency.
...
`src/components/footer.tsx` is a **server component** — no interactivity, so no JavaScript sent to the browser:
...
- **Config file access** (`.env`, `.git`, `.htaccess`)
- **WordPress scanner probes** (`/wp-admin`, `/wp-login.php`)
- **Database admin panel probes** (`/phpmyadmin`, `/adminer`)
...
Vibe coding is not "AI writes, you ship." It's "AI writes, you review, you iterate." Claude Code wrote excellent code for this site, but I still reviewed every component, questioned design decisions, and caught a few things I wanted done differently. The AI is the engine. You're the driver.
</Warning>
