...
![Two Bugs Past the Goal Line: Phase 1.1 title slide. 5 surfaces, 2 security signals, 694 pytest, 24/24 Playwright, Wave 0 found 3 UDM v9+ wire-shape surprises.](/blog/home-network-mission-control-dashboard-phase-1-1/slides/slide-01.webp)

![The full /network tab on the live dashboard: stat strip across the top, VLAN cards row, port-forward table, ZBF zone matrix, SSID-to-VLAN Sankey, and topology graph. Cyber theme palette, all five surfaces fed from one poll.](/blog/home-network-mission-control-dashboard-phase-1-1/network-full-page.png)
...
![VLAN cards row: one card per UniFi network with subnet, gateway, DHCP range, IGMP snooping flag, and a live client-count chip.](/blog/home-network-mission-control-dashboard-phase-1-1/vlan-cards.png)
...
![UDM v9+ deployment analysis infographic: Wave 0 wire-shape surprises, the 5-wave deployment funnel catching the CRITICAL ship-blocker, and the two post-merge silent-zero bugs caught by live in-session verification.](/blog/home-network-mission-control-dashboard-phase-1-1/udm-v9-gotchas-infographic.webp)

![ZBF zone matrix with a question-mark glyph in cells the dashboard can't verify due to UDM v9+ ID encoding. Color-coded for allow, block, and unknown, with a footer disclaimer explaining the limitation.](/blog/home-network-mission-control-dashboard-phase-1-1/zone-matrix.png)
...
![Port-forward table on the live dashboard with the source-is-any column flagged red on two rows: qBittorrent and a folx Mac. The port_forward_open signal evaluates RED on the live LAN.](/blog/home-network-mission-control-dashboard-phase-1-1/port-forward-table.png)
...
![SSID to VLAN Sankey diagram on the live dashboard, with thick flow bands from each SSID into the VLAN it lands on, and a final flow band into the live client-count chip on each VLAN.](/blog/home-network-mission-control-dashboard-phase-1-1/ssid-vlan-sankey.png)
...
![Security signals row on the live dashboard after PR #15: port_forward_open evaluates RED with value 2.0, gravity_stale evaluates GREEN with value 2.03 days. Both signals reading honestly against the LAN.](/blog/home-network-mission-control-dashboard-phase-1-1/security-signals-row.png)
...
![Topology graph on the live dashboard with a synthetic WAN node inserted at the top, edges flowing down to the gateway, switches, access points, and clients. Force-directed D3 layout, color-coded by device type.](/blog/home-network-mission-control-dashboard-phase-1-1/topology-graph.png)
